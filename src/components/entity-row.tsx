...
export function EntityRow({ entity, editMode, departmentId, onRemoved, onUpdated }: EntityRowProps) {
  ...
  const color = entity.entityType?.color ?? "#a855f7";
  const propEntries = Object.entries(entity.properties).slice(0, 3);

  return (
    <div className={`rounded-lg transition ${expanded ? "bg-white/[0.02] border border-white/[0.06]" : ""}`}>
      {/* Collapsed row */}
      <div className="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-white/[0.03] transition">
        ...
        {!expanded && propEntries.map(([k, v]) => (
          <span key={k} className="text-[10px] text-white/25 truncate max-w-[100px] hidden sm:inline">{v}</span>
        ))}
        {entity.sourceSystem && (
      ...
      {expanded && editMode && (
        <div className="px-4 pb-4 pt-2 space-y-4">
          {/* Properties */}
          <div>
            <p className="text-[10px] uppercase tracking-wider text-white/40 mb-2">Properties</p>
            {Object.keys(editingProps).length === 0 ? (
              <p className="text-xs text-white/25">No properties</p>
            ) : (
              <div className="space-y-1.5">
                {Object.entries(editingProps).map(([key, value]) => (
                  <div key={key} className="flex items-center gap-3">
                    <span className="text-xs text-white/40 w-28 flex-shrink-0 truncate">{prettifySlug(key)}</span>
                    <input
                      value={value}
                      onChange={(e) => setEditingProps((prev) => ({ ...prev, [key]: e.target.value }))}
                      className="flex-1 bg-white/5 border border-white/10 rounded px-2 py-1 text-xs text-white/80 outline-none focus:border-purple-500/40"
                    />
                  </div>
                ))}
              </div>
            ...
            {Object.keys(editingProps).length > 0 && (
              <div className="flex justify-end gap-2 mt-3">
                <button onClick={saveProperties} disabled={saving} className="text-[11px] text-purple-400 hover:text-purple-300 disabled:opacity-50">
                  {saving ? "Saving..." : "Save Changes"}
                </button>
                <button onClick={() => setExpanded(false)} className="text-[11px] text-white/40 hover:text-white/60">Cancel</button>
              </div>
            )}
          </div>
          ...
          <div>
            ...
            {loadingRels ? (
              ...
              <p className="text-xs text-white/25">No relationships</p>
            ) : (
              <div className="space-y-1">
                {relationships.map((rel) => {
                  ...
                  return (
                    <div key={rel.id} className="flex items-center gap-2 px-2 py-1.5 rounded hover:bg-white/[0.03] transition text-xs">
                      <span className="text-white/30">{isOutgoing ? "\u2192" : "\u2190"}</span>
                      <span className="text-white/50">{rel.relationshipType.name}</span>
                      <span className="text-white/70 flex-1 truncate">{linkedEntity?.displayName ?? "Unknown"}</span>
                      <button
                        onClick={() => removeRelationship(rel.id)}
                        className="w-4 h-4 rounded bg-red-500/10 hover:bg-red-500/20 flex items-center justify-center transition flex-shrink-0"
          ...
          <div className="border-t border-white/[0.06] pt-3 flex items-center gap-4">
            {confirmRemove ? (
              <div className="flex-1">
                <p className="text-xs text-white/50 mb-2">This will unlink the entity from this department. The entity will not be deleted.</p>
                <div className="flex gap-2">
                  <button onClick={removeFromDepartment} className="text-[11px] text-red-400 hover:text-red-300">Confirm Remove</button>
                  <button onClick={() => setConfirmRemove(false)} className="text-[11px] text-white/40 hover:text-white/60">Cancel</button>
                </div>
              </div>
            ) : confirmDelete ? (
              <div className="flex-1">
                <p className="text-xs text-red-400/80 mb-2">Permanently delete this entity and all its relationships? This cannot be undone.</p>
                <div className="flex gap-2">
                  <button onClick={deleteEntity} className="text-[11px] text-red-400 hover:text-red-300">Delete Permanently</button>
                  <button onClick={() => setConfirmDelete(false)} className="text-[11px] text-white/40 hover:text-white/60">Cancel</button>
                </div>
              </div>
            ) : (
              <>
                <button onClick={() => setConfirmRemove(true)} className="text-[11px] text-red-400/70 hover:text-red-400 transition">
                  Remove from department
                </button>
                <button onClick={() => setConfirmDelete(true)} className="text-[11px] text-red-400/50 hover:text-red-400 transition">
                  Delete entity
                </button>
              </>
            )}
