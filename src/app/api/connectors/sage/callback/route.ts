...
export async function GET(req: NextRequest) {
  ...
  const oauthReturn = cookieStore.get("sage_oauth_return")?.value;
  cookieStore.delete("sage_oauth_return");
  ...
  if (!code || !state) {
    return NextResponse.redirect(
      new URL(`${returnBase}${sep}sage=error&reason=missing_params`, APP_BASE),
    );
  }
  ...
  if (!storedState || storedState !== state) {
    return NextResponse.redirect(
      new URL(`${returnBase}${sep}sage=error&reason=invalid_state`, APP_BASE),
    );
  }
  ...
  if (!clientId || !clientSecret) {
    return NextResponse.redirect(
      new URL(`${returnBase}${sep}sage=error&reason=server_config`, APP_BASE),
    );
  }

  // Exchange code for tokens
  const tokenResp = await fetch("https://oauth.accounting.sage.com/token", {
    method: "POST",
  ...
  if (!tokenResp.ok) {
    ...
    console.error("[sage-oauth] Token exchange failed:", errBody);
    return NextResponse.redirect(
      new URL(`${returnBase}${sep}sage=error&reason=token_exchange`, APP_BASE),
    );
  }
  ...
  const existing = await prisma.sourceConnector.findFirst({
    where: { operatorId, userId: null, provider: "sage", ...ACTIVE_CONNECTOR },
  });
  ...
  const capProvider = getProvider("sage");
  if (capProvider) {
    registerConnectorCapabilities(connectorId, operatorId, capProvider).catch((err) =>
      console.error("[sage-oauth] Failed to register write capabilities:", err),
    );
  }

  return NextResponse.redirect(
    new URL(`${returnBase}${sep}sage=connected`, APP_BASE),
  );
}
