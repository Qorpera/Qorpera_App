...
function DepartmentDetailInner() {
  ...
  const deptId = params.domainId as string;
  ...
  const load = useCallback(async () => {
    try {
      const [domainRes, membersRes, linksRes] = await Promise.all([
        fetchApi(`/api/domains/${deptId}`),
        fetchApi(`/api/domains/${deptId}/members`),
        fetchApi(`/api/domains/${deptId}/external-links`),
      ]);
      if (domainRes.status === 403) { router.push("/map"); return; }
      if (!domainRes.ok) { setNotFound(true); return; }
      setDept(await domainRes.json());
  ...
  const loadDocs = useCallback(async () => {
    const res = await fetchApi(`/api/domains/${deptId}/documents`);
    if (res.ok) setDocsData(await res.json());
  }, [deptId]);

  const fetchExternalLinks = useCallback(async () => {
    const res = await fetchApi(`/api/domains/${deptId}/external-links`);
    if (res.ok) {
  ...
  const fetchConnectedEntities = useCallback(async (append = false) => {
    ...
    const res = await fetchApi(`/api/domains/${deptId}/connected-entities?limit=50&offset=${currentOffset}`);
    if (res.ok) {
      const data = await res.json();
  ...
  async function saveDeptField(field: "name" | "description", value: string) {
    const res = await fetchApi(`/api/domains/${deptId}`, {
      method: "PATCH",
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    setAddError("");
    try {
      const res = await fetchApi(`/api/domains/${deptId}/members`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async function saveEdit() {
    if (!editId) return;
    setEditError("");
    const res = await fetchApi(`/api/domains/${deptId}/members/${editId}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  async function confirmRemove() {
    if (!removeId) return;
    const res = await fetchApi(`/api/domains/${deptId}/members/${removeId}`, { method: "DELETE" });
    if (res.ok) {
  ...
  async function uploadFile(file: File, documentType: string) {
    ...
    try {
      ...
      const res = await fetchApi(`/api/domains/${deptId}/documents/upload`, {
        method: "POST",
        body: form,
      });
      ...
      if (isSlot) {
        ...
        try {
          const extRes = await fetch(
            `/api/domains/${deptId}/documents/${created.id}/extract`,
            { method: "POST" },
          );
          if (extRes.ok) {
  ...
  async function handleReExtract(docId: string, slotType: string) {
    ...
    setDocError("");
    try {
      const res = await fetchApi(`/api/domains/${deptId}/documents/${docId}/extract`, { method: "POST" });
      if (res.ok) {
  ...
  async function handleReprocess(docId: string) {
    setDocError("");
    const res = await fetchApi(`/api/domains/${deptId}/documents/${docId}/reprocess`, { method: "POST" });
    if (!res.ok) {
      const err = await res.json().catch(() => null);
  ...
  async function handleDeleteDoc(docId: string) {
    const res = await fetchApi(`/api/domains/${deptId}/documents/${docId}`, { method: "DELETE" });
    if (res.ok) {
      await loadDocs();
    }
    setDeleteDocId(null);
  ...
  async function handleConfirmDiff() {
    ...
    try {
      const res = await fetch(
        `/api/domains/${deptId}/documents/${diffModal.docId}/confirm`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      if (res.ok) {
        ...
        // Refresh members
        const membersRes = await fetchApi(`/api/domains/${deptId}/members`);
        if (membersRes.ok) setMembers(await membersRes.json());
      } else {
        const err = await res.json().catch(() => null);
  ...
  if (notFound || !dept) {
    return (
      <AppShell>
        <div className="p-8">
          <p className="text-[var(--fg2)] mb-4">{t("domainNotFound")}</p>
  ...
  return (
    <AppShell>
      <div className="max-w-4xl mx-auto p-8 space-y-8">
        ...
        {/* ---- Department Knowledge (populated by wiki synthesis) ---- */}
        <Section title={t("structuralDocuments")}>
          <p style={{ fontSize: 13, color: "var(--fg3)", fontStyle: "italic", padding: "8px 0" }}>
            Department knowledge is being organized automatically.
            Check back after the system has processed your connected data.
          </p>
        </Section>
